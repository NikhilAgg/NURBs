...
class WriteResults:
    def __init__(self, typ, bspline_ind, param_ind):
        self.filename = pth / f"results/results_{typ}{bspline_ind}{param_ind}.txt"
    ...
    def write_results(self, x_points, y_points, omegas, dw, ep_step):
        with open(self.filename, "w") as f:
            f.write(self.lines)
...
def taylor_test(ro, ri, l, lc, bspline_ind, param_ind, typ, ep_step, ep_list, fil):
    ...
    for i in range(1, 6):
        ...
        fil.write_results(x_points, y_points, omegas, dw, ep_step)
...
ri = 0.25
l = 1
lcs = [1e-1, 9e-2, 8e-2, 7e-2, 6e-2, 5e-2]
