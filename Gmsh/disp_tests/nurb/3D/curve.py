...
def volume(y, n):
    intg = []
    for i in range(n):
        intg.append(np.trapz(y[i*n:(i+1)*n, 2], x=y[i*n:(i+1)*n, 1]))

    return np.trapz(intg, x=y[0::n, 0])
...
disp = (norms.T*(dp[:, 0]*epsilon_point[0] + dp[:, 1]*epsilon_point[1]+ dp[:, 2]*epsilon_point[2])).T
# ax.scatter3D(y[:, 0] + disp[:, 0], y[:, 1] + disp[:, 1], y[:, 2] + disp[:, 2])

disp_w = (norms.T*(dw*epsilon_w)).T
# ax.scatter3D(y[:, 0] + disp[:, 0], y[:, 1] + disp[:, 1], y[:, 2] + disp[:, 2])
...
# ax.scatter3D(y_new_p[:, 0], y_new_p[:, 1], y_new_p[:, 2])
...
# ax.scatter3D(y_new_w[:, 0], y_new_w[:, 1], y_new_w[:, 2])
