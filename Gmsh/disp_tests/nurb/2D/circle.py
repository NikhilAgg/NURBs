...
ti = time.time()
dy = np.array([np.linalg.norm(circle.get_displacement("control point", u, 0)) for u in t])
for i in range(1, 8):
    p = np.array([np.linalg.norm(circle.get_displacement("control point", u, i)) for u in t])
    dy += p
